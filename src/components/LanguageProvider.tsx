"use client";

import { createContext, useContext, useEffect, useMemo, useState } from "react";

export type AppLanguage = "de" | "en";

type TranslationTree = {
  [key: string]: string | TranslationTree;
};

const translations = {
  de: {
    app: {
      brand: "FyNest",
    },
    nav: {
      home: "Startseite",
      monthlyPayments: "Monatsübersicht",
      loans: "Kredit",
      insurances: "Versicherungen",
      incomes: "Einnahmen",
      expenses: "Ausgaben",
      projects: "Gemeinsame Ausgaben",
      savings: "Sparen",
      investments: "Investitionen",
      shoppingList: "Einkaufsliste",
      exchange: "Wechsel",
      tools: "Werkzeuge",
      contracts: "Verträge",
      general: "Allgemein",
      settings: "Einstellungen",
      openMenu: "Menu offnen",
      closeMenu: "Menu schliessen",
      main: "Hauptnavigation",
      signedInAs: "Angemeldet als",
      owner: "Inhaber",
      readonly: "Nebenbenutzer",
      readwrite: "Nebenbenutzer"
    },
    pages: {
      home: {
        eyebrow: "Startseite",
      },
      monthlyPayments: {
        eyebrow: "Monatsübersicht",
      },
      loans: {
        eyebrow: "Kredite",
      },
      insurances: {
        eyebrow: "Versicherungen",
      },
      incomes: {
        eyebrow: "Einnahmen",
      },
      expenses: {
        eyebrow: "Ausgaben",
      },
      projects: {
        eyebrow: "Gemeinsame Ausgaben",
      },
      investments: {
        eyebrow: "Investitionen",
      },
      exchange: {
        eyebrow: "Wechsel",
      },
      shoppingList: {
        eyebrow: "Einkaufsliste",
      },
      savings: {
        eyebrow: "Sparen",
      },
      contractsGeneral: {
        eyebrow: "Verträge",
      },
      settings: {
        eyebrow: "Einstellungen",
      }
    },
    dashboard: {
      monthPicker: "Monat auswahlen",
      previousMonth: "Vorheriger Monat",
      nextMonth: "Nachster Monat",
      currentMonth: "Aktueller Monat",
      selectedMonth: "Ausgewahlter Monat",
      today: "Heute",
      relaxed: "Entspannt",
      attentive: "Aufmerksam bleiben",
      critical: "Kritisch",
      noIncome: "Keine Einnahmen",
      used: "genutzt",
      income: "Einnahmen",
      previousBalance: "Vormonat",
      payments: "Zahlungen",
      monthlyExpenses: "Monatliche Ausgaben",
      currentBookings: "Aktuelle Buchungen",
      loanAmount: "Kreditbetrag",
      activeLoans: "aktive Kredite",
      insurances: "Versicherungen",
      monthlyPremiums: "Monatliche Pramien",
      freeAmount: "Freier Betrag",
      investmentOverview: "Investment-Ubersicht",
      investmentItems: "Positionen",
      investedTotal: "Gekauft",
      currentInvestmentValue: "Wert heute",
      returnRate: "Rendite",
      investmentResult: "Ergebnis",
      savings: "Sparen",
      savingsTotal: "Gesamtes Sparguthaben",
      monthlyOverview: "Monatsübersicht",
      viewMonthlyPayments: "Monatsübersicht öffnen",
      shoppingList: "Einkaufsliste",
      openShoppingItems: "Offene Einkäufe",
      viewShoppingList: "Einkaufsliste öffnen",
      loadingPayments: "Zahlungen werden geladen...",
      noPayments: "Fur diesen Monat sind noch keine Zahlungen vorhanden.",
      paymentTypeLoan: "Kredit",
      paymentTypeInsurance: "Versicherung",
      paymentTypeExpense: "Ausgabe",
      paymentTypeContract: "Vertrag",
      markPaid: "als bezahlt markieren",
      updating: "Aktualisiert...",
      paid: "Bezahlt",
      partiallyPaid: "Teilweise bezahlt",
      open: "Offen",
      paidAmount: "Bezahlt",
      savePaidAmount: "bezahlten Betrag speichern"
    },
    common: {
      actions: "Aktionen",
      add: "hinzufugen",
      edit: "bearbeiten",
      delete: "loschen",
      save: "Speichern",
      saving: "Speichern...",
      cancel: "Abbrechen",
      deleting: "Loschen...",
      closeDialog: "Dialog schliessen",
      description: "Beschreibung",
      descriptionPlaceholder: "Notiz, Zweck oder wichtige Details",
      selectPlaceholder: "Bitte auswahlen",
      total: "Summe",
      day: "Tag",
      paymentInterval: "Zahlungsrhythmus",
      everyMonth: "Jeden Monat",
      everyThreeMonths: "Alle 3 Monate",
      everySixMonths: "Alle 6 Monate",
      everyYear: "Jahrlich",
      paymentInterval1: "Monatlich",
      paymentInterval3: "Alle 3 Monate",
      paymentInterval6: "Alle 6 Monate",
      paymentInterval12: "Jahrlich",
      oneTime: "Einmalig",
      recurring: "Wiederkehrend"
    },
    loading: {
      data: "Daten werden geladen",
      wait: "Bitte warten..."
    },
    auth: {
      preferences: "Darstellung und Sprache",
      loginTitle: "Anmelden",
      loginDescription: "Bitte melde dich an, um deine Finanzdaten zu verwalten.",
      loginAction: "Anmelden",
      loggingIn: "Anmelden...",
      loginFailed: "Login fehlgeschlagen.",
      forgotPassword: "Passwort vergessen?",
      noAccount: "Noch kein Konto?",
      registerAction: "Registrieren",
      registerTitle: "Registrieren",
      registerDescription: "Erstelle dein eigenes Konto, offne den Telegram-Bot und bestatige den Code.",
      usernameOrTelegram: "Benutzername oder Telegram-Nummer",
      transferWarning: "Dieser Benutzer hat bereits Zugriff auf ein anderes Konto.",
      registrationFailed: "Registrierung fehlgeschlagen.",
      codeFailed: "Code konnte nicht bestatigt werden.",
      accessRemovedTitle: "Zugriff wird entfernt",
      preparing: "Vorbereiten...",
      confirmAndContinue: "Bestatigen und fortfahren",
      startRegistration: "Registrierung starten",
      openTelegramTitle: "Telegram offnen",
      openTelegramDescription: "Offne den Bot, tippe auf Start und gib danach den Code hier ein.",
      openTelegram: "Telegram Bot offnen",
      telegramUsernameMissing: "TELEGRAM_BOT_USERNAME fehlt in den Environment Variables.",
      telegramCode: "Telegram-Code",
      checking: "Prufen...",
      finishRegistration: "Registrierung abschliessen",
      alreadyRegistered: "Schon registriert?",
      forgotTitle: "Passwort vergessen",
      forgotDescription: "Fordere einen Telegram-Code an und lege danach ein neues Passwort fest.",
      resetCodeSent: "Ein Code wurde an deinen Telegram-Bot gesendet.",
      resetNoContact: "Wenn kein Code ankommt, ist fur diesen Benutzer noch kein Telegram-Kontakt gespeichert.",
      resetStartFailed: "Reset konnte nicht gestartet werden.",
      passwordChangeFailed: "Passwort konnte nicht geandert werden.",
      sending: "Senden...",
      sendCode: "Code senden",
      newPassword: "Neues Passwort",
      savePassword: "Passwort speichern",
      backTo: "Zuruck zum"
    },
    loans: {
      add: "Kredit hinzufugen",
      loading: "Kredite werden geladen...",
      loan: "Kredit",
      bank: "Bank",
      amount: "Kreditbetrag",
      totalInterest: "Gesamtzinsen",
      rate: "Rate",
      monthlyRate: "Monatsrate",
      interest: "Zins",
      interestRate: "Zinssatz",
      firstPayment: "Erste Zahlung",
      status: "Status",
      details: "Details anzeigen",
      addTitle: "Neuen Kredit hinzufugen",
      editTitle: "Kredit bearbeiten",
      name: "Kreditname",
      description: "Beschreibung",
      descriptionPlaceholder: "Notiz, Zweck oder wichtige Details zum Kredit",
      amortization: "Tilgungsplan",
      installments: "Anzahl Raten",
      date: "Datum",
      remainingAfter: "Rest danach",
      deleteLabel: "Kredit loschen",
      deleteTitle: "Kredit wirklich loschen?",
      deleteText: "wird aus der aktuellen Tabelle entfernt."
    },
    insurances: {
      add: "Versicherung hinzufugen",
      loading: "Versicherungen werden geladen...",
      insurance: "Versicherung",
      provider: "Anbieter",
      coverage: "Schutz",
      amount: "Betrag",
      monthly: "Monatlich",
      debitDay: "Abbuchungstag",
      firstDebitDate: "Erste Abbuchung",
      startDate: "Startdatum",
      endDate: "Enddatum",
      renewal: "Verlangerung",
      noTerm: "Keine Laufzeit",
      addTitle: "Neue Versicherung hinzufugen",
      editTitle: "Versicherung bearbeiten",
      noRenewal: "Keine Verlangerung / kein Ablaufdatum",
      deleteLabel: "Versicherung loschen",
      deleteTitle: "Versicherung wirklich loschen?",
      deleteText: "wird aus der aktuellen Tabelle entfernt."
    },
    incomes: {
      add: "Einnahme hinzufugen",
      loading: "Einnahmen werden geladen...",
      ariaTabs: "Einnahmenarten",
      recurringTab: "Feste Einnahmen",
      oneTimeTab: "Einmalige Eingange",
      title: "Titel",
      source: "Quelle",
      amount: "Betrag",
      date: "Datum",
      type: "Typ",
      fixed: "Fest",
      empty: "Keine Einnahmen in dieser Kategorie vorhanden.",
      addTitle: "Neue Einnahme hinzufugen",
      editTitle: "Einnahme bearbeiten",
      deleteLabel: "Einnahme loschen",
      deleteTitle: "Einnahme wirklich loschen?",
      recurringMonthly: "Feste monatliche Einnahme",
      entryDay: "Eingangstag",
      previousBalance: "Vormonat",
      previousBalanceAutomatic: "Automatisch aus dem Vormonat berechnet",
      previousBalanceManual: "Manuell festgelegt"
    },
    expenses: {
      add: "Neue Ausgabe",
      loading: "Ausgaben werden geladen...",
      title: "Titel",
      category: "Kategorie",
      date: "Datum",
      amount: "Betrag",
      empty: "Keine Ausgaben in dieser Kategorie vorhanden.",
      addTitle: "Neue Ausgabe eintragen",
      editTitle: "Ausgabe bearbeiten",
      deleteLabel: "Ausgabe loschen",
      deleteTitle: "Ausgabe wirklich loschen?",
      saveError: "Die Ausgabe konnte nicht gespeichert werden."
    },
    investments: {
      add: "Investition hinzufugen",
      loading: "Investitionen werden geladen...",
      investment: "Investition",
      addTitle: "Neue Investition eintragen",
      editTitle: "Investition bearbeiten",
      asset: "Wert",
      symbol: "Symbol",
      quantity: "Anzahl",
      purchasePrice: "Kaufpreis (EUR)",
      currentPrice: "Aktueller Preis",
      currentValue: "Aktueller Wert",
      result: "Gewinn / Verlust",
      purchaseDate: "Kaufdatum",
      investedTotal: "Investiert",
      currentTotal: "Aktueller Wert",
      priceUnavailable: "Nicht verfugbar",
      empty: "Noch keine Investitionen vorhanden.",
      deleteLabel: "Investition loschen",
      deleteTitle: "Investition wirklich loschen?",
      deleteText: "wird aus deinem Depot entfernt."
    },
    exchange: {
      liveTitle: "Live-Kurs",
      pair: "USDT / Toman",
      loading: "Tetherpreis wird geladen...",
      refresh: "Aktualisieren",
      refreshing: "Ladt...",
      lastPrice: "Letzter Preis",
      updatedAt: "Aktualisiert",
      bestBid: "Bester Kauf",
      bestAsk: "Bester Verkauf",
      spread: "Spread",
      lastTrade: "Letzter Trade",
      fetchedAt: "Abgerufen",
      source: "Quelle",
      toman: "Toman",
      error: "Der Tetherpreis konnte nicht geladen werden.",
      calculator: {
        eyebrow: "Rechner",
        title: "Euro und Toman umrechnen",
        mode: "Rechenrichtung",
        eurToToman: "Euro zu Toman",
        tomanToEur: "Toman zu Euro",
        euroInput: "Euro, die du an KuCoin sendest",
        tomanInput: "Toman, die auf deinem Konto ankommen sollen",
        approxReceive: "Du erhaltst ca.",
        approxSend: "Du sendest ca.",
        tomanResult: "Kommt auf deinem Konto an",
        euroResult: "Du musst an KuCoin senden",
        feeDetails: "Gebuhren-Details",
        feeHint: "Inklusive 1 EUR KuCoin-Gebuehr, 1,5 USDT Transfer und 15.000 Toman Bankgebuehr.",
        afterKucoinFee: "Nach KuCoin-Gebuehr",
        boughtUsdt: "Gekaufte USDT",
        afterTransfer: "Nach Transfergebuehr",
        beforeBankFee: "Vor Bankgebuehr",
        withBankFee: "Ziel plus Bankgebuehr",
        requiredUsdt: "Benotigte USDT",
        withTransferFee: "Mit Transfergebuehr",
        beforeKucoinFee: "Vor KuCoin-Gebuehr",
        rateNote: "Berechnet mit"
      }
    },
    shoppingList: {
      add: "Eintrag hinzufügen",
      addTitle: "Neuen Einkauf hinzufügen",
      edit: "Eintrag bearbeiten",
      editTitle: "Einkauf bearbeiten",
      name: "Was möchtest du kaufen?",
      quantity: "Menge",
      unit: "Einheit",
      hasDeadline: "Frist festlegen",
      deadline: "Frist",
      loading: "Einkaufsliste wird geladen...",
      loadingSuggestions: "Gespeicherte Einträge werden geladen...",
      openItems: "Offene Einkäufe",
      empty: "Deine Einkaufsliste ist leer.",
      completedOn: "Erledigt am",
      markDone: "Als erledigt markieren",
      markOpen: "Wieder öffnen",
      delete: "Eintrag löschen",
      share: "Teilen",
      shareTitle: "Einkaufsliste",
      selectItemsToShare: "Einträge zum Teilen auswählen",
      selectForShare: "Zum Teilen auswählen",
      deselectForShare: "Auswahl aufheben",
      selected: "ausgewählt",
      selectAll: "Alle auswählen",
      clearSelection: "Auswahl aufheben",
      shareSelected: "Auswahl senden",
      totalItems: "Einträge insgesamt",
      copiedToClipboard: "Die Einkaufsliste wurde in die Zwischenablage kopiert.",
      shareError: "Die Einkaufsliste konnte nicht geteilt werden.",
      error: "Die Aktion konnte nicht abgeschlossen werden.",
      units: { kg: "Kilo", package: "Packung", piece: "Stück", bottle: "Flasche" }
    },
    savings: {
      add: "Spartopf hinzufugen",
      loading: "Spartopfe werden geladen...",
      saving: "Spartopf",
      addTitle: "Neuen Spartopf eintragen",
      editTitle: "Spartopf bearbeiten",
      goal: "Spartopf",
      targetAmount: "Zielbetrag",
      currentAmount: "Aktueller Betrag",
      monthlyContribution: "Monatlich sparen",
      targetDate: "Zieldatum",
      progress: "Fortschritt",
      savedTotal: "Gespart",
      targetTotal: "Ziel gesamt",
      details: "Details",
      detailsTitle: "Buchungen",
      deposit: "Einzahlen",
      withdraw: "Auszahlen",
      depositTitle: "Geld einzahlen",
      withdrawTitle: "Geld auszahlen",
      editTransactionTitle: "Sparbuchung bearbeiten",
      transactionAmount: "Betrag",
      transactionDate: "Datum",
      transactionType: "Typ",
      loadingTransactions: "Buchungen werden geladen...",
      noTransactions: "Noch keine Buchungen vorhanden.",
      transactionDeleteLabel: "Sparbuchung loschen",
      transactionDeleteTitle: "Sparbuchung wirklich loschen?",
      transactionDeleteText: "wird aus diesem Spartopf entfernt.",
      manageOnlyInSavings: "Nur unter Sparen bearbeitbar",
      depositHint: "Die Einzahlung wird automatisch als Ausgabe gespeichert.",
      withdrawHint: "Die Auszahlung wird automatisch als Einnahme gespeichert.",
      empty: "Noch keine Spartopfe vorhanden.",
      deleteLabel: "Spartopf loschen",
      deleteTitle: "Spartopf wirklich loschen?",
      deleteText: "wird aus deiner Sparplanung entfernt."
    },
    projects: {
      addProject: "Projekt hinzufügen",
      editProject: "Projekt bearbeiten",
      projectDetails: "Projektdetails",
      deleteProject: "Projekt löschen",
      deleteProjectTitle: "Projekt wirklich löschen?",
      deleteProjectText: "und alle zugehörigen Ausgaben werden dauerhaft entfernt.",
      loading: "Projekte werden geladen...",
      empty: "Noch keine Projekte vorhanden.",
      openProject: "Projekt öffnen",
      shareProject: "Projekt teilen",
      copiedToClipboard: "Das Projekt wurde in die Zwischenablage kopiert.",
      shareFailed: "Das Projekt konnte nicht geteilt werden.",
      calculateSettlement: "Ausgleich berechnen",
      settlementTitle: "Optimaler Ausgleich",
      settlementHint: "Mit diesen direkten Überweisungen werden alle Salden ausgeglichen.",
      transferCount: "Überweisungen",
      paysTo: "zahlt an",
      alreadySettled: "Alles ist ausgeglichen",
      alreadySettledHint: "Aktuell ist keine Überweisung erforderlich.",
      shareSettlement: "Ausgleich teilen",
      settlementCopied: "Der Ausgleich wurde in die Zwischenablage kopiert.",
      startDate: "Startdatum",
      categories: "Kategorien",
      category: "Kategorie",
      categoryName: "Name der Kategorie",
      addCategory: "Kategorie hinzufügen",
      members: "Mitglieder",
      memberName: "Name des Mitglieds",
      addMember: "Mitglied hinzufügen",
      shareWeight: "Anteil",
      shareHint: "Relative Zahl, z. B. 1, 1.5 oder 2",
      removeItem: "Eintrag entfernen",
      expenseCount: "Ausgaben",
      totalExpenses: "Gesamtausgaben",
      addExpense: "Ausgabe hinzufügen",
      editExpense: "Ausgabe bearbeiten",
      deleteExpense: "Ausgabe löschen",
      deleteExpenseTitle: "Ausgabe wirklich löschen?",
      deleteExpenseText: "wird aus diesem Projekt entfernt.",
      amount: "Betrag",
      date: "Datum",
      paidBy: "Bezahlt von",
      expenseDescription: "Beschreibung",
      expenseDescriptionPlaceholder: "z. B. Hotel, Restaurant oder Zugticket",
      noExpenses: "Noch keine Ausgaben in diesem Projekt.",
      overview: "Projektübersicht",
      memberBalances: "Anteile der Mitglieder",
      paid: "Bezahlt",
      expected: "Eigener Anteil",
      balance: "Saldo",
      categorySpending: "Ausgaben nach Kategorie",
      expenseHistory: "Ausgabenverlauf",
      noDescription: "Keine Beschreibung",
      operationFailed: "Die Aktion konnte nicht abgeschlossen werden.",
      atLeastOneCategory: "Mindestens eine Kategorie ist erforderlich.",
      atLeastOneMember: "Mindestens ein Mitglied ist erforderlich."
    },
    settings: {
      themeLabel: "Theme",
      themeTitle: "Farbmodus",
      themeGroup: "Farbmodus auswahlen",
      languageLabel: "Sprache",
      languageTitle: "App-Sprache",
      languageGroup: "Sprache auswahlen",
      notificationsLabel: "Benachrichtigungen",
      notificationsTitle: "Einkaufslisten-Updates",
      notificationsDescription: "Erhalte eine Push-Benachrichtigung, wenn ein verbundener Benutzer einen Einkauf hinzufügt.",
      notificationsGroup: "Push-Benachrichtigungen ein- oder ausschalten",
      notificationsEnabled: "Benachrichtigungen sind auf diesem Gerät eingeschaltet.",
      notificationsDisabled: "Benachrichtigungen sind auf diesem Gerät ausgeschaltet.",
      notificationsBlocked: "Benachrichtigungen sind im Browser blockiert. Erlaube sie in den Browser-Einstellungen.",
      notificationsUnsupported: "Dieser Browser unterstützt keine Push-Benachrichtigungen. Auf dem iPhone muss FyNest zuerst zum Home-Bildschirm hinzugefügt werden.",
      notificationsUnconfigured: "Push-Benachrichtigungen sind auf dem Server noch nicht konfiguriert.",
      notificationsLoading: "Status wird geprüft...",
      notificationsError: "Die Benachrichtigungseinstellung konnte nicht geändert werden.",
      on: "Ein",
      off: "Aus",
      accessLabel: "Zugriff",
      accessTitle: "Benutzer und Berechtigungen",
      addUser: "Benutzer hinzufugen",
      addUserTitle: "Neuen Benutzer hinzufugen",
      editUserTitle: "Benutzer bearbeiten",
      username: "Benutzername",
      password: "Passwort",
      passwordUnchanged: "Leer lassen, wenn unverandert",
      permission: "Berechtigung",
      allowedActions: "Erlaubte Aktionen",
      actionsAllowed: "Aktionen erlaubt",
      allActions: "Alle Aktionen",
      readonly: "Readonly",
      readwrite: "Read & Write",
      managePermissions: "Aktionen verwalten",
      permissionsDescription: "Wähle genau die Aktionen aus, die dieser Benutzer ausführen darf.",
      selectAll: "Alle auswählen",
      clearAll: "Alle abwählen",
      permissionSections: { monthlyPayments: "Monatsübersicht", incomes: "Einnahmen", expenses: "Ausgaben", savings: "Sparen", investments: "Investitionen", loans: "Kredite", insurances: "Versicherungen", contracts: "Verträge", projects: "Projekte", shopping: "Einkaufsliste" },
      permissionActions: { create: "Erstellen", edit: "Bearbeiten", delete: "Löschen", updatePayments: "Zahlungen abhaken oder Betrag ändern", updateCarryover: "Vormonatssaldo ändern", deposit: "Einzahlung buchen", withdraw: "Auszahlung buchen", editTransaction: "Sparbuchung bearbeiten", deleteTransaction: "Sparbuchung löschen", createProjectExpense: "Projektausgabe erstellen", editProjectExpense: "Projektausgabe bearbeiten", deleteProjectExpense: "Projektausgabe löschen", complete: "Einkauf abhaken oder wieder öffnen" },
      noUsers: "Noch keine freigegebenen Benutzer vorhanden.",
      deleteUserLabel: "Benutzer loschen",
      deleteUserTitle: "Benutzer wirklich loschen?",
      deleteUserText: "verliert den Zugriff auf deine Daten.",
      logout: "Logout",
      loggingOut: "Abmelden..."
    },
    contracts: {
      generalLabel: "Allgemein",
      generalTitle: "Weitere Verträge",
      generalDescription: "Hier kann später eine eigene Tabelle für allgemeine Verträge wie Mobilfunk, Streaming oder Mitgliedschaften entstehen.",
      add: "Vertrag hinzufügen",
      loading: "Verträge werden geladen...",
      title: "Titel",
      provider: "Anbieter",
      category: "Kategorie",
      amount: "Betrag",
      monthlyAmount: "Monatlich",
      debitDay: "Abbuchungstag",
      startDate: "Startdatum",
      endDate: "Enddatum",
      noEndDate: "Keine Laufzeit / kein Enddatum",
      status: "Status",
      note: "Notiz",
      active: "Aktiv",
      paused: "Pausiert",
      cancelled: "Gekündigt",
      addTitle: "Neuen Vertrag hinzufügen",
      editTitle: "Vertrag bearbeiten",
      deleteLabel: "Vertrag löschen",
      deleteTitle: "Vertrag wirklich löschen?",
      deleteText: "wird aus der aktuellen Tabelle entfernt.",
      empty: "Noch keine allgemeinen Verträge vorhanden."
    }
  },
  en: {
    app: {
      brand: "Finance Manager",
    },
    nav: {
      home: "Home",
      monthlyPayments: "Monthly overview",
      loans: "Credit",
      insurances: "Insurance",
      incomes: "Income",
      expenses: "Expenses",
      projects: "Shared Expenses",
      savings: "Savings",
      investments: "Investments",
      shoppingList: "Shopping List",
      exchange: "Exchange",
      tools: "Tools",
      contracts: "Contracts",
      general: "General",
      settings: "Settings",
      openMenu: "Open menu",
      closeMenu: "Close menu",
      main: "Main navigation",
      signedInAs: "Signed in as",
      owner: "Owner",
      readonly: "Shared user",
      readwrite: "Shared user"
    },
    pages: {
      home: {
        eyebrow: "Home",
      },
      monthlyPayments: {
        eyebrow: "Monthly overview",
      },
      loans: {
        eyebrow: "Loans",
      },
      insurances: {
        eyebrow: "Insurance",
      },
      incomes: {
        eyebrow: "Income",
      },
      expenses: {
        eyebrow: "Expenses",
      },
      projects: {
        eyebrow: "Shared expenses",
      },
      investments: {
        eyebrow: "Investments",
      },
      exchange: {
        eyebrow: "Exchange",
      },
      shoppingList: {
        eyebrow: "Shopping list",
      },
      savings: {
        eyebrow: "Savings",
      },
      contractsGeneral: {
        eyebrow: "Contracts",
      },
      settings: {
        eyebrow: "Settings",
      }
    },
    dashboard: {
      monthPicker: "Select month",
      previousMonth: "Previous month",
      nextMonth: "Next month",
      currentMonth: "Current month",
      selectedMonth: "Selected month",
      today: "Today",
      relaxed: "Relaxed",
      attentive: "Stay alert",
      critical: "Critical",
      noIncome: "No income",
      used: "used",
      income: "Income",
      previousBalance: "Previous month",
      payments: "Payments",
      monthlyExpenses: "Monthly expenses",
      currentBookings: "Current bookings",
      loanAmount: "Loan amount",
      activeLoans: "active loans",
      insurances: "Insurance",
      monthlyPremiums: "Monthly premiums",
      freeAmount: "Free amount",
      investmentOverview: "Investment overview",
      investmentItems: "positions",
      investedTotal: "Bought",
      currentInvestmentValue: "Value today",
      returnRate: "Return",
      investmentResult: "Result",
      savings: "Savings",
      savingsTotal: "Total saved",
      monthlyOverview: "Monthly overview",
      viewMonthlyPayments: "Open monthly overview",
      shoppingList: "Shopping list",
      openShoppingItems: "Open shopping items",
      viewShoppingList: "Open shopping list",
      loadingPayments: "Loading payments...",
      noPayments: "There are no payments for this month yet.",
      paymentTypeLoan: "Loan",
      paymentTypeInsurance: "Insurance",
      paymentTypeExpense: "Expense",
      paymentTypeContract: "Contract",
      markPaid: "mark as paid",
      updating: "Updating...",
      paid: "Paid",
      partiallyPaid: "Partially paid",
      open: "Open",
      paidAmount: "Paid",
      savePaidAmount: "save paid amount"
    },
    common: {
      actions: "Actions",
      add: "add",
      edit: "edit",
      delete: "delete",
      save: "Save",
      saving: "Saving...",
      cancel: "Cancel",
      deleting: "Deleting...",
      closeDialog: "Close dialog",
      description: "Description",
      descriptionPlaceholder: "Note, purpose or important details",
      selectPlaceholder: "Please select",
      total: "Total",
      day: "day",
      paymentInterval: "Payment interval",
      everyMonth: "Every month",
      everyThreeMonths: "Every 3 months",
      everySixMonths: "Every 6 months",
      everyYear: "Yearly",
      paymentInterval1: "Monthly",
      paymentInterval3: "Every 3 months",
      paymentInterval6: "Every 6 months",
      paymentInterval12: "Yearly",
      oneTime: "One-time",
      recurring: "Recurring"
    },
    loading: {
      data: "Loading data",
      wait: "Please wait..."
    },
    auth: {
      preferences: "Appearance and language",
      loginTitle: "Sign in",
      loginDescription: "Sign in to manage your financial data.",
      loginAction: "Sign in",
      loggingIn: "Signing in...",
      loginFailed: "Login failed.",
      forgotPassword: "Forgot password?",
      noAccount: "No account yet?",
      registerAction: "Register",
      registerTitle: "Register",
      registerDescription: "Create your own account, open the Telegram bot and confirm the code.",
      usernameOrTelegram: "Username or Telegram number",
      transferWarning: "This user already has access to another account.",
      registrationFailed: "Registration failed.",
      codeFailed: "Code could not be confirmed.",
      accessRemovedTitle: "Access will be removed",
      preparing: "Preparing...",
      confirmAndContinue: "Confirm and continue",
      startRegistration: "Start registration",
      openTelegramTitle: "Open Telegram",
      openTelegramDescription: "Open the bot, tap Start and then enter the code here.",
      openTelegram: "Open Telegram bot",
      telegramUsernameMissing: "TELEGRAM_BOT_USERNAME is missing in the environment variables.",
      telegramCode: "Telegram code",
      checking: "Checking...",
      finishRegistration: "Finish registration",
      alreadyRegistered: "Already registered?",
      forgotTitle: "Forgot password",
      forgotDescription: "Request a Telegram code and then set a new password.",
      resetCodeSent: "A code was sent to your Telegram bot.",
      resetNoContact: "If no code arrives, this user does not have a stored Telegram contact yet.",
      resetStartFailed: "Reset could not be started.",
      passwordChangeFailed: "Password could not be changed.",
      sending: "Sending...",
      sendCode: "Send code",
      newPassword: "New password",
      savePassword: "Save password",
      backTo: "Back to"
    },
    loans: {
      add: "Add credit",
      loading: "Loading credits...",
      loan: "Credit",
      bank: "Bank",
      amount: "Credit amount",
      totalInterest: "Total interest",
      rate: "Installment",
      monthlyRate: "Monthly installment",
      interest: "Interest",
      interestRate: "Interest rate",
      firstPayment: "First payment",
      status: "Status",
      details: "show details",
      addTitle: "Add new credit",
      editTitle: "Edit credit",
      name: "Credit name",
      description: "Description",
      descriptionPlaceholder: "Note, purpose or important credit details",
      amortization: "Repayment plan",
      installments: "Installments",
      date: "Date",
      remainingAfter: "Remaining after",
      deleteLabel: "Delete credit",
      deleteTitle: "Delete this credit?",
      deleteText: "will be removed from the current table."
    },
    insurances: {
      add: "Add insurance",
      loading: "Loading insurance...",
      insurance: "Insurance",
      provider: "Provider",
      coverage: "Coverage",
      amount: "Amount",
      monthly: "Monthly",
      debitDay: "Debit day",
      firstDebitDate: "First debit",
      startDate: "Start date",
      endDate: "End date",
      renewal: "Renewal",
      noTerm: "No term",
      addTitle: "Add new insurance",
      editTitle: "Edit insurance",
      noRenewal: "No renewal / no expiry date",
      deleteLabel: "Delete insurance",
      deleteTitle: "Delete this insurance?",
      deleteText: "will be removed from the current table."
    },
    incomes: {
      add: "Add income",
      loading: "Loading income...",
      ariaTabs: "Income types",
      recurringTab: "Fixed income",
      oneTimeTab: "One-time inflows",
      title: "Title",
      source: "Source",
      amount: "Amount",
      date: "Date",
      type: "Type",
      fixed: "Fixed",
      empty: "No income exists in this category.",
      addTitle: "Add new income",
      editTitle: "Edit income",
      deleteLabel: "Delete income",
      deleteTitle: "Delete this income?",
      recurringMonthly: "Fixed monthly income",
      entryDay: "Entry day",
      previousBalance: "Previous month",
      previousBalanceAutomatic: "Calculated automatically from the previous month",
      previousBalanceManual: "Set manually"
    },
    expenses: {
      add: "New expense",
      loading: "Loading expenses...",
      title: "Title",
      category: "Category",
      date: "Date",
      amount: "Amount",
      empty: "No expenses exist in this category.",
      addTitle: "Add new expense",
      editTitle: "Edit expense",
      deleteLabel: "Delete expense",
      deleteTitle: "Delete this expense?",
      saveError: "The expense could not be saved."
    },
    investments: {
      add: "Add investment",
      loading: "Loading investments...",
      investment: "Investment",
      addTitle: "Add new investment",
      editTitle: "Edit investment",
      asset: "Asset",
      symbol: "Symbol",
      quantity: "Quantity",
      purchasePrice: "Purchase price (EUR)",
      currentPrice: "Current price",
      currentValue: "Current value",
      result: "Gain / loss",
      purchaseDate: "Purchase date",
      investedTotal: "Invested",
      currentTotal: "Current value",
      priceUnavailable: "Unavailable",
      empty: "No investments yet.",
      deleteLabel: "Delete investment",
      deleteTitle: "Delete this investment?",
      deleteText: "will be removed from your portfolio."
    },
    exchange: {
      liveTitle: "Live rate",
      pair: "USDT / Toman",
      loading: "Loading Tether price...",
      refresh: "Refresh",
      refreshing: "Loading...",
      lastPrice: "Last price",
      updatedAt: "Updated",
      bestBid: "Best bid",
      bestAsk: "Best ask",
      spread: "Spread",
      lastTrade: "Last trade",
      fetchedAt: "Fetched",
      source: "Source",
      toman: "Toman",
      error: "The Tether price could not be loaded.",
      calculator: {
        eyebrow: "Calculator",
        title: "Convert Euro and Toman",
        mode: "Calculation direction",
        eurToToman: "Euro to Toman",
        tomanToEur: "Toman to Euro",
        euroInput: "Euro you send to KuCoin",
        tomanInput: "Toman you want to receive",
        approxReceive: "You receive approx.",
        approxSend: "You send approx.",
        tomanResult: "Arrives in your account",
        euroResult: "You need to send to KuCoin",
        feeDetails: "Fee details",
        feeHint: "Includes 1 EUR KuCoin fee, 1.5 USDT transfer fee and 15,000 Toman bank fee.",
        afterKucoinFee: "After KuCoin fee",
        boughtUsdt: "Bought USDT",
        afterTransfer: "After transfer fee",
        beforeBankFee: "Before bank fee",
        withBankFee: "Target plus bank fee",
        requiredUsdt: "Required USDT",
        withTransferFee: "With transfer fee",
        beforeKucoinFee: "Before KuCoin fee",
        rateNote: "Calculated with"
      }
    },
    shoppingList: {
      add: "Add item",
      addTitle: "Add shopping item",
      edit: "Edit item",
      editTitle: "Edit shopping item",
      name: "What do you want to buy?",
      quantity: "Quantity",
      unit: "Unit",
      hasDeadline: "Set a deadline",
      deadline: "Deadline",
      loading: "Loading shopping list...",
      loadingSuggestions: "Loading saved items...",
      openItems: "Open items",
      empty: "Your shopping list is empty.",
      completedOn: "Completed on",
      markDone: "Mark as completed",
      markOpen: "Reopen item",
      delete: "Delete item",
      share: "Share",
      shareTitle: "Shopping list",
      selectItemsToShare: "Select items to share",
      selectForShare: "Select for sharing",
      deselectForShare: "Remove from selection",
      selected: "selected",
      selectAll: "Select all",
      clearSelection: "Clear selection",
      shareSelected: "Send selection",
      totalItems: "Total items",
      copiedToClipboard: "The shopping list was copied to the clipboard.",
      shareError: "The shopping list could not be shared.",
      error: "The action could not be completed.",
      units: { kg: "kg", package: "package", piece: "piece", bottle: "bottle" }
    },
    savings: {
      add: "Add savings fund",
      loading: "Loading savings funds...",
      saving: "Savings fund",
      addTitle: "Add new savings fund",
      editTitle: "Edit savings fund",
      goal: "Savings fund",
      targetAmount: "Target amount",
      currentAmount: "Current amount",
      monthlyContribution: "Monthly saving",
      targetDate: "Target date",
      progress: "Progress",
      savedTotal: "Saved",
      targetTotal: "Total target",
      details: "Details",
      detailsTitle: "Transactions",
      deposit: "Deposit",
      withdraw: "Withdraw",
      depositTitle: "Deposit money",
      withdrawTitle: "Withdraw money",
      editTransactionTitle: "Edit savings transaction",
      transactionAmount: "Amount",
      transactionDate: "Date",
      transactionType: "Type",
      loadingTransactions: "Loading transactions...",
      noTransactions: "No transactions yet.",
      transactionDeleteLabel: "Delete savings transaction",
      transactionDeleteTitle: "Delete this savings transaction?",
      transactionDeleteText: "will be removed from this savings fund.",
      manageOnlyInSavings: "Editable only in Savings",
      depositHint: "The deposit is automatically saved as an expense.",
      withdrawHint: "The withdrawal is automatically saved as income.",
      empty: "No savings funds yet.",
      deleteLabel: "Delete savings fund",
      deleteTitle: "Delete this savings fund?",
      deleteText: "will be removed from your savings plan."
    },
    projects: {
      addProject: "Add project",
      editProject: "Edit project",
      projectDetails: "Project details",
      deleteProject: "Delete project",
      deleteProjectTitle: "Delete this project?",
      deleteProjectText: "and all related expenses will be permanently removed.",
      loading: "Loading projects...",
      empty: "No projects yet.",
      openProject: "Open project",
      shareProject: "Share project",
      copiedToClipboard: "The project was copied to the clipboard.",
      shareFailed: "The project could not be shared.",
      calculateSettlement: "Calculate settlement",
      settlementTitle: "Optimized settlement",
      settlementHint: "These direct transfers settle every member balance.",
      transferCount: "transfers",
      paysTo: "pays",
      alreadySettled: "Everything is settled",
      alreadySettledHint: "No transfer is currently required.",
      shareSettlement: "Share settlement",
      settlementCopied: "The settlement was copied to the clipboard.",
      startDate: "Start date",
      categories: "Categories",
      category: "Category",
      categoryName: "Category name",
      addCategory: "Add category",
      members: "Members",
      memberName: "Member name",
      addMember: "Add member",
      shareWeight: "Share",
      shareHint: "Relative number, e.g. 1, 1.5 or 2",
      removeItem: "Remove item",
      expenseCount: "Expenses",
      totalExpenses: "Total expenses",
      addExpense: "Add expense",
      editExpense: "Edit expense",
      deleteExpense: "Delete expense",
      deleteExpenseTitle: "Delete this expense?",
      deleteExpenseText: "will be removed from this project.",
      amount: "Amount",
      date: "Date",
      paidBy: "Paid by",
      expenseDescription: "Description",
      expenseDescriptionPlaceholder: "e.g. hotel, restaurant or train ticket",
      noExpenses: "No expenses in this project yet.",
      overview: "Project overview",
      memberBalances: "Member shares",
      paid: "Paid",
      expected: "Own share",
      balance: "Balance",
      categorySpending: "Spending by category",
      expenseHistory: "Expense history",
      noDescription: "No description",
      operationFailed: "The action could not be completed.",
      atLeastOneCategory: "At least one category is required.",
      atLeastOneMember: "At least one member is required."
    },
    settings: {
      themeLabel: "Theme",
      themeTitle: "Color mode",
      themeGroup: "Select color mode",
      languageLabel: "Language",
      languageTitle: "App language",
      languageGroup: "Select language",
      notificationsLabel: "Notifications",
      notificationsTitle: "Shopping list updates",
      notificationsDescription: "Receive a push notification when a connected user adds a shopping item.",
      notificationsGroup: "Turn push notifications on or off",
      notificationsEnabled: "Notifications are enabled on this device.",
      notificationsDisabled: "Notifications are disabled on this device.",
      notificationsBlocked: "Notifications are blocked in the browser. Allow them in the browser settings.",
      notificationsUnsupported: "This browser does not support push notifications. On iPhone, add FyNest to the Home Screen first.",
      notificationsUnconfigured: "Push notifications have not been configured on the server yet.",
      notificationsLoading: "Checking status...",
      notificationsError: "The notification setting could not be changed.",
      on: "On",
      off: "Off",
      accessLabel: "Access",
      accessTitle: "Users and permissions",
      addUser: "Add user",
      addUserTitle: "Add new user",
      editUserTitle: "Edit user",
      username: "Username",
      password: "Password",
      passwordUnchanged: "Leave empty to keep unchanged",
      permission: "Permission",
      allowedActions: "Allowed actions",
      actionsAllowed: "actions allowed",
      allActions: "All actions",
      readonly: "Readonly",
      readwrite: "Read & Write",
      managePermissions: "Manage actions",
      permissionsDescription: "Select exactly which actions this user is allowed to perform.",
      selectAll: "Select all",
      clearAll: "Clear all",
      permissionSections: { monthlyPayments: "Monthly overview", incomes: "Income", expenses: "Expenses", savings: "Savings", investments: "Investments", loans: "Loans", insurances: "Insurance", contracts: "Contracts", projects: "Projects", shopping: "Shopping list" },
      permissionActions: { create: "Create", edit: "Edit", delete: "Delete", updatePayments: "Check payments or change amount", updateCarryover: "Change previous-month balance", deposit: "Add deposit", withdraw: "Add withdrawal", editTransaction: "Edit savings transaction", deleteTransaction: "Delete savings transaction", createProjectExpense: "Create project expense", editProjectExpense: "Edit project expense", deleteProjectExpense: "Delete project expense", complete: "Complete or reopen shopping item" },
      noUsers: "No shared users yet.",
      deleteUserLabel: "Delete user",
      deleteUserTitle: "Delete this user?",
      deleteUserText: "will lose access to your data.",
      logout: "Logout",
      loggingOut: "Logging out..."
    },
    contracts: {
      generalLabel: "General",
      generalTitle: "Other contracts",
      generalDescription: "A dedicated table for general contracts such as mobile plans, streaming or memberships can be added here later.",
      add: "Add contract",
      loading: "Loading contracts...",
      title: "Title",
      provider: "Provider",
      category: "Category",
      amount: "Amount",
      monthlyAmount: "Monthly",
      debitDay: "Debit day",
      startDate: "Start date",
      endDate: "End date",
      noEndDate: "No term / no end date",
      status: "Status",
      note: "Note",
      active: "Active",
      paused: "Paused",
      cancelled: "Cancelled",
      addTitle: "Add new contract",
      editTitle: "Edit contract",
      deleteLabel: "Delete contract",
      deleteTitle: "Delete this contract?",
      deleteText: "will be removed from the current table.",
      empty: "No general contracts yet."
    }
  }
} as const;

type TranslationPath = string;

type LanguageContextValue = {
  language: AppLanguage;
  setLanguage: (language: AppLanguage) => void;
  t: (path: TranslationPath) => string;
};

const LanguageContext = createContext<LanguageContextValue | null>(null);

function readTranslation(language: AppLanguage, path: TranslationPath) {
  const value = path.split(".").reduce<string | TranslationTree | undefined>((current, key) => {
    if (!current || typeof current === "string") {
      return undefined;
    }

    return current[key];
  }, translations[language]);

  return typeof value === "string" ? value : path;
}

export function LanguageProvider({ children }: { children: React.ReactNode }) {
  const [language, setLanguageState] = useState<AppLanguage>("de");

  useEffect(() => {
    const savedLanguage = window.localStorage.getItem("finance-language");
    if (savedLanguage === "de" || savedLanguage === "en") {
      setLanguageState(savedLanguage);
      document.documentElement.lang = savedLanguage;
      document.documentElement.dataset.language = savedLanguage;
    }
  }, []);

  function setLanguage(nextLanguage: AppLanguage) {
    setLanguageState(nextLanguage);
    document.documentElement.lang = nextLanguage;
    document.documentElement.dataset.language = nextLanguage;
    window.localStorage.setItem("finance-language", nextLanguage);
  }

  const value = useMemo(
    () => ({
      language,
      setLanguage,
      t: (path: TranslationPath) => readTranslation(language, path)
    }),
    [language]
  );

  return <LanguageContext.Provider value={value}>{children}</LanguageContext.Provider>;
}

export function useLanguage() {
  const context = useContext(LanguageContext);

  if (!context) {
    throw new Error("useLanguage must be used within LanguageProvider");
  }

  return context;
}
